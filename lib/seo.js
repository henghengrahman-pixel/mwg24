...
function normalizeCanonical(baseUrl = "", canonicalOrPath = "") {
  ...
  const raw = absoluteUrl(safeBase, canonicalOrPath || "/");
  ...
  try {
    ...
    cleaned = url.toString().replace(/\/$/, "");
    return cleaned || safeBase;
  } catch {
    return cleaned.replace(/\/$/, "") || safeBase;
  }
}
...
function buildPageId(baseUrl, path = "/") {
...
function breadcrumbSchema(items = [], baseUrl = "", url = "/") {
  const normalized = ensureArray(items)
    ...
    .map((item, index) => ({
      ...
      item: normalizeCanonical(baseUrl, item.url)
    }))
    .filter(item => item.name && item.item);

  if (!normalized.length) return null;

  const pageUrl = normalizeCanonical(baseUrl, url || "/");
...
function webpageSchema({
  ...
  const breadcrumb = breadcrumbSchema(breadcrumbItems, safeBase, url || "/");
  if (breadcrumb) {
    schema.breadcrumb = {
      "@id": `${pageUrl}#breadcrumb`
    };
