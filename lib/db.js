...
function runMigrations() {
  ...
  // perbarui artikel lama yang kolom status-nya kosong
  db.exec(`
    UPDATE articles
    SET status = 'publish'
  ...
  // rapikan settings lama yang masih pakai title lama
  db.exec(`
    UPDATE settings
    SET
  ...
  if (!beritaCount) {
    const insert = _db.prepare(`
      INSERT INTO articles (
        title, slug, excerpt, content, image, category, status, meta_title, meta_description
      ) VALUES (
        @title, @slug, @excerpt, @content, @image, @category, @status, @meta_title, @meta_description
      )
    `);
