...
const artikelUpload = multer({ storage: storageFor("artikel") });
...
router.get("/", (req, res) => {
  ...
  res.render("admin-dashboard", {
    seo: buildSeo({ title: "Dashboard Admin | Wisata Berastagi", noindex: true }),
    stats: {
      wisata: db.prepare("SELECT COUNT(*) as total FROM wisata").get().total,
      villa: db.prepare("SELECT COUNT(*) as total FROM villa").get().total,
      articles: db.prepare("SELECT COUNT(*) as total FROM articles").get().total,
...
router.get("/wisata", (req, res) => {
  ...
  res.render("admin-wisata", {
    seo: buildSeo({ title: "Admin Wisata | Wisata Berastagi", noindex: true }),
    items
  });
});
...
router.get("/wisata/new", (req, res) => {
  res.render("admin-wisata-form", {
    seo: buildSeo({ title: "Tambah Wisata | Wisata Berastagi", noindex: true }),
    item: null
  });
});
...
router.get("/wisata/edit/:id", (req, res) => {
  ...
  res.render("admin-wisata-form", {
    seo: buildSeo({ title: "Edit Wisata | Wisata Berastagi", noindex: true }),
    item
  });
});
...
router.post("/wisata/save", wisataUpload.single("image"), (req, res) => {
  ...
  const image = req.file ? fileUrl("wisata", req.file.filename) : String(req.body.current_image || "").trim();
  const payload = {
    title,
    slug,
  ...
  if (id) {
    db.prepare(`
      ...
      SET title=@title, slug=@slug, excerpt=@excerpt, content=@content, image=@image,
          location=@location, ticket_price=@ticket_price, open_hours=@open_hours, maps_url=@maps_url,
...
router.get("/villa", (req, res) => {
  ...
  res.render("admin-villa", {
    seo: buildSeo({ title: "Admin Villa | Wisata Berastagi", noindex: true }),
    items
  });
});
...
router.get("/villa/new", (req, res) => {
  res.render("admin-villa-form", {
    seo: buildSeo({ title: "Tambah Villa | Wisata Berastagi", noindex: true }),
    item: null
  });
});
...
router.get("/villa/edit/:id", (req, res) => {
  ...
  res.render("admin-villa-form", {
    seo: buildSeo({ title: "Edit Villa | Wisata Berastagi", noindex: true }),
    item
  });
});
...
router.post("/villa/save", villaUpload.single("image"), (req, res) => {
  ...
  const content = String(req.body.content || "").trim();
  const slug = makeSlug(req.body.slug || title);
  const image = req.file ? fileUrl("villa", req.file.filename) : String(req.body.current_image || "").trim();
  const payload = {
    title,
    slug,
    excerpt: String(req.body.excerpt || excerpt(content, 150)).trim(),
  ...
  if (id) {
    db.prepare(`
      ...
      SET title=@title, slug=@slug, excerpt=@excerpt, content=@content, image=@image,
          ...
          meta_description=@meta_description, is_featured=@is_featured, updated_at=CURRENT_TIMESTAMP
      WHERE id=${id}
    `).run(payload);
  } else {
    db.prepare(`
...
router.get("/articles", (req, res) => {
  ...
  res.render("admin-articles", {
    seo: buildSeo({ title: "Admin Artikel | Wisata Berastagi", noindex: true }),
    items
  });
});
...
router.get("/articles/new", (req, res) => {
  res.render("admin-article-form", {
    seo: buildSeo({ title: "Tambah Artikel | Wisata Berastagi", noindex: true }),
    item: null
  });
});
...
router.get("/articles/edit/:id", (req, res) => {
  ...
  res.render("admin-article-form", {
    seo: buildSeo({ title: "Edit Artikel | Wisata Berastagi", noindex: true }),
    item
  });
});

router.post("/articles/save", artikelUpload.single("image"), (req, res) => {
  const db = getDb();
  const id = req.body.id ? Number(req.body.id) : null;
  const title = String(req.body.title || "").trim();
  const content = String(req.body.content || "").trim();
  const slug = makeSlug(req.body.slug || title);
  const image = req.file ? fileUrl("artikel", req.file.filename) : String(req.body.current_image || "").trim();
  const payload = {
    title,
    slug,
    excerpt: String(req.body.excerpt || excerpt(content, 150)).trim(),
    content,
    image,
    category: String(req.body.category || "wisata").trim(),
    meta_title: String(req.body.meta_title || `${title} | Wisata Berastagi`).trim(),
    meta_description: String(req.body.meta_description || excerpt(content, 150)).trim()
  };
...
router.get("/comments", (req, res) => {
  ...
  res.render("admin-comments", {
    seo: buildSeo({ title: "Komentar Pengunjung | Wisata Berastagi", noindex: true }),
    items
  });
});
...
router.get("/settings", (req, res) => {
  res.render("admin-settings", {
    seo: buildSeo({ title: "Pengaturan Website | Wisata Berastagi", noindex: true }),
...
router.post("/settings", (req, res) => {
  getDb().prepare(`
    UPDATE settings
    SET site_name = ?, site_tagline = ?, contact_phone = ?, contact_email = ?,
        address = ?, footer_text = ?, homepage_title = ?, homepage_meta_description = ?,
