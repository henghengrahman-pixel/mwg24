...
router.get("/login", (req, res) => {
  ...
  res.render("login", {
    error: "",
    seo: { title: "Login Admin | Wisata Berastagi", noindex: true }
  });
});
...
router.get("/", (req, res) => {
  res.render("admin-dashboard", {
    ...
    seo: { title: "Dashboard Admin | Wisata Berastagi", noindex: true }
  });
});
...
router.post("/upload-image", upload.single("image"), (req, res) => {
  ...
  return res.json({
    location: `/uploads/berita/${req.file.filename}`
  });
});
...
function listRoute(pathname, table, view) {
  router.get(pathname, (req, res) => {
    const items = getDb()
      .prepare(`SELECT * FROM ${table} ORDER BY updated_at DESC, id DESC`)
      .all();

    res.render(view, {
      items,
      seo: { title: `Admin ${table}`, noindex: true }
    });
  });
}
...
router.get("/gallery", (req, res) => {
  const items = getDb()
    .prepare("SELECT * FROM gallery ORDER BY sort_order ASC, updated_at DESC, id DESC")
    .all();
...
function editRender(pathname, table, view, empty = {}) {
  router.get(`${pathname}/new`, (req, res) => {
    res.render(view, {
      item: empty,
      error: "",
      items: [],
  ...
  router.get(`${pathname}/edit/:id`, (req, res) => {
    ...
    if (!item) return res.redirect(adminPath(pathname));
...
router.post("/wisata/save", (req, res) => {
  ...
  const data = {
    ...
    image: normalizeImageInput(
      ...
      String(req.body.current_image || "").trim() || "/images/wisata-berastagi-cover.jpg"
    ),
  ...
  if (!title || !slug) {
    return res.render("admin-wisata-form", {
      item: { ...req.body, image: req.body.image_url || req.body.image || req.body.current_image || "" },
  ...
  if (id) {
    db.prepare(`
      UPDATE wisata
      SET title=@title, slug=@slug, excerpt=@excerpt, content=@content, image=@image, location=@location,
          ticket_price=@ticket_price, open_hours=@open_hours, maps_url=@maps_url,
          meta_title=@meta_title, meta_description=@meta_description, is_featured=@is_featured,
...
router.post("/kuliner/save", (req, res) => {
  ...
  if (!title || !slug) {
    return res.render("admin-kuliner-form", {
      item: { ...req.body, image: req.body.image_url || req.body.image || req.body.current_image || "" },
      error: "Judul wajib diisi.",
      seo: { noindex: true }
    });
  ...
  if (id) {
    db.prepare(`
      UPDATE kuliner
      SET title=@title, slug=@slug, excerpt=@excerpt, content=@content, image=@image, location=@location,
          price_range=@price_range, open_hours=@open_hours, contact_phone=@contact_phone, maps_url=@maps_url,
          meta_title=@meta_title, meta_description=@meta_description, is_featured=@is_featured,
          updated_at=CURRENT_TIMESTAMP
      WHERE id=@id
    `).run({ id, ...data });
    ...
    db.prepare(`
      INSERT INTO kuliner (
        title, slug, excerpt, content, image, location, price_range, open_hours,
        contact_phone, maps_url, meta_title, meta_description, is_featured
      ) VALUES (
        @title,@slug,@excerpt,@content,@image,@location,@price_range,@open_hours,
        @contact_phone,@maps_url,@meta_title,@meta_description,@is_featured
      )
...
router.post("/villa/save", (req, res) => {
  ...
  const image = normalizeImageInput(
    req.body.image_url || req.body.image,
    String(req.body.current_image || "").trim() || "/images/wisata-berastagi-cover.jpg"
  );

  const rawGallery = req.body.gallery_image_urls || req.body.images || "";
  const galleryImages = Array.from(new Set([image, ...parseMultiUrls(rawGallery)])).filter(Boolean);
  ...
  if (!title || !slug) {
    return res.render("admin-villa-form", {
      item: {
        ...req.body,
        image,
  ...
  if (id) {
    db.prepare(`
      UPDATE villa
      SET title=@title, slug=@slug, excerpt=@excerpt, content=@content, image=@image, images=@images,
          price=@price, location=@location, facilities=@facilities, booking_url=@booking_url,
          contact_phone=@contact_phone, maps_url=@maps_url, meta_title=@meta_title,
          meta_description=@meta_description, is_featured=@is_featured,
          updated_at=CURRENT_TIMESTAMP
      WHERE id=@id
    `).run({ id, ...data });
  } else {
    db.prepare(`
      INSERT INTO villa (
        title, slug, excerpt, content, image, images, price, location, facilities,
        booking_url, contact_phone, maps_url, meta_title, meta_description, is_featured
      ) VALUES (
        @title,@slug,@excerpt,@content,@image,@images,@price,@location,@facilities,
        @booking_url,@contact_phone,@maps_url,@meta_title,@meta_description,@is_featured
      )
...
router.post("/berita/save", upload.single("image"), (req, res) => {
  ...
  const image = uploadedImage || normalizeImageInput(
    req.body.image_url || req.body.image,
    String(req.body.current_image || "").trim() || "/images/wisata-berastagi-cover.jpg"
  );
  ...
  if (id) {
    db.prepare(`
      UPDATE articles
      SET title=@title, slug=@slug, excerpt=@excerpt, content=@content, image=@image, category=@category,
    ...
    db.prepare(`
      INSERT INTO articles (
        title, slug, excerpt, content, image, category, status,
...
router.post("/gallery/save", (req, res) => {
  ...
  if (id) {
    db.prepare(`
      UPDATE gallery
      SET title=@title, slug=@slug, image=@image, alt_text=@alt_text, caption=@caption,
          sort_order=@sort_order, is_active=@is_active, updated_at=CURRENT_TIMESTAMP
      WHERE id=@id
    `).run({ id, ...data });
  } else {
    db.prepare(`
      INSERT INTO gallery (title, slug, image, alt_text, caption, sort_order, is_active)
...
function deleteRoute(pathname, table) {
  router.post(`${pathname}/delete/:id`, (req, res) => {
    ...
    res.redirect(adminPath(pathname));
  });
}

deleteRoute("/wisata", "wisata");
deleteRoute("/villa", "villa");
deleteRoute("/kuliner", "kuliner");
deleteRoute("/berita", "articles");
deleteRoute("/gallery", "gallery");
...
router.post("/settings", (req, res) => {
  getDb().prepare(`
    UPDATE settings
    SET site_name=?, site_tagline=?, contact_phone=?, contact_email=?, address=?, footer_text=?,
        homepage_title=?, homepage_meta_description=?, hero_title=?, hero_subtitle=?,
